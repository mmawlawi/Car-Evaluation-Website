...
function updateModels() {
    ...
    var firstVisible = true; // Tracks the first visible option to auto-select it

    for (var i = 0; i < options.length; i++) {
        var option = options[i];
        var model_brand_id = option.getAttribute("brand_id");

        // If "Other" brand is selected, show only "Other" model
        if (brand_id === "other") {
            option.style.display = (model_brand_id === "Other") ? "block" : "none";
            if (model_brand_id === "Other" && firstVisible) {
                firstVisible = false;
            }
        } else {
            // Show model if it belongs to the selected brand or it is the "Other" option
    ...
    // Additional logic to handle 'other' model input field visibility
...
function validateEngineFuelKm() {
    ...
    if (fuel < 3 || fuel > 30) {
        fuelError.textContent =
            "Fuel consumption must be between 3 and 30 L/100km.";
